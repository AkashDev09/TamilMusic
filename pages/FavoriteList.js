...
import { Dimensions, FlatList, Image, ImageBackground, NativeModules, SafeAreaView, StyleSheet, Text, TextInput, TouchableOpacity, View, } from 'react-native'
...
function FavoriteList({ navigation }) {
    ...
    const AutoRow = ({ item, index }) => {
        return (
            ...
            <TouchableOpacity key={index} onPress={() => { dispatch(SelectItem({ songs: item, RouterN: "Search", play: true, destroyPair: item.Id !== selectItem?.songs?.Id ? true : false })); playsougFunction(item.streamURL, dispatch); thunmbnail(item.streamURL) }}>
        ...
        <View style={style.Home_con}>
            ...
            <View style={{ ...style.search_body, height: Object.keys(selectItem) > 0 && route.name === "Search" ? Math.floor(Dimensions.get('window').height * 0.8) : Object.keys(selectItem) > 0 && route.name === "Player" ? Math.floor(Dimensions.get('window').height * 0.7) : Math.floor(Dimensions.get('window').height * 0.8) }}>
                {filteredList.length === 0 ? (
                    <View style={style.Noitem}>
                        <Image
                            style={style.noItemImage}
                            source={require('../Assets/Images/Empty-amico.png')}
                        />
                        <Text style={{ color: "tomato", fontSize: 20, fontWeight: '800' }}> No More</Text>
                    </View>
                ) : (
                    <SafeAreaView>
                        <FlatList
...
const style = StyleSheet.create({
    Noitem: {
        ...
        height: Math.floor(Dimensions.get('window').height * 0.9),
